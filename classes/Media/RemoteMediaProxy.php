...
final class RemoteMediaProxy
{
    ...
    /**
     * Allocate a private staging filename for a supported upload producer.
     *
     * @param string $relativePath Upload-relative destination that determines the native editor's extension.
     * @return string|null Request-owned staging file, or null when unsupported or storage is unavailable.
     */
    public function stageUpload(string $relativePath): ?string
    {
        if (!$this->isValidPath($relativePath) || $this->getMimeType($relativePath) === null) {
            return null;
        }
        return (new FileCache('remote-media'))->stage(strtolower(pathinfo($relativePath, PATHINFO_EXTENSION)));
    }

    /**
     * Retain a producer-validated upload in the same cache used by browser and PHP readers.
     *
     * @param string       $relativePath Upload-relative identity of the completed derivative.
     * @param string       $file         Validated private staging file owned by this request.
     * @param integer|null $expiresAt    Source deadline that must not be extended by generation.
     * @return boolean Whether the completed file was adopted; failure leaves it request-owned.
     */
    public function storeUpload(string $relativePath, string $file, ?int $expiresAt = null): bool
    {
        $type = $this->getMimeType($relativePath);
        if (!$this->isValidPath($relativePath) || $type === null || ($expiresAt !== null && $expiresAt <= time())) {
            return false;
        }
        $remote = $this->remoteIdentity($relativePath, $type);
        $uploads = wp_get_upload_dir();
        if ($remote === null || file_exists($uploads['basedir'] . '/' . $relativePath)) {
            return false;
        }
        $cache = new FileCache('remote-media', strtolower(pathinfo($relativePath, PATHINFO_EXTENSION)));
        $createdAt = $expiresAt === null ? null : $expiresAt - self::MAX_AGE;
        return $cache->publish($remote['key'], $file, $createdAt) !== null;
    }
