...
Coverage includes signed URLs and tampering, local precedence, expiry and source isolation, 404-only native generation, crop failure, inferred/rounded dimensions and crop positions, binary response failures and route casing, private cache publication, reader independence, and cleanup. Retina characterization covers local scaling (0.5×, 1×, 1.5×, 2×), local derivative reuse, exact remote-file access, and the current missing-original limitation. It does not claim remote Retina generation support. Animated-GIF assertions run only when Imagick is available. The suite is tested with Timber 1.22.0, 2.3.3 and 2.4.1.

CI uses Composer constraints `^1.0` and `^2.0` to resolve the latest compatible stable release of each major line. Each job uses a fresh dependency directory without an existing lockfile and a fresh WordPress/MySQL environment, and logs the resolved Timber version. Only the Timber 1 job allows security-advisory exceptions for legacy `twig/twig` 1.x/2.x in its temporary Composer configuration. An explicit audit prints those ignored advisories and still fails on other security advisories; abandoned legacy packages are reported without failing the audit. Its unused `composer/installers` plugin is disabled. Neither Timber 2 nor the plugin's own dependencies receive these exceptions. These fixtures are excluded from release packages. They are not yet comprehensive Apache lifecycle, encryption/settings, concurrency, or Multisite/domain-mapping tests.
